...
function send_register_email_subscriber()
{
	...
	$json_obj = json_encode(array("email" => $email, 'click_origin' => $click_origin, 'apiKey' => $apiKey));

//	echo $json_obj;


	$ch = curl_init();
	curl_setopt($ch, CURLOPT_URL, "https://admin.meediaonne.com/api/landing-page/subscriber/store");
	curl_setopt($ch, CURLOPT_POST, 1);
	curl_setopt($ch, CURLOPT_USERAGENT, 'Mediapartner-SeaStorm-1.0');
	curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
	curl_setopt($ch, CURLOPT_POSTFIELDS, $json_obj);
...
function prefix_register_send_form()
{
	...
	register_rest_route('email-subscriber', '/send', array(
		'methods'  => 'POST',
		'callback' => 'send_register_email_subscriber',
	));
}
