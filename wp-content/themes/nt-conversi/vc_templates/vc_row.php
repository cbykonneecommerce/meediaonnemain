...
if( $nt_conversi_bg_position != '' ) {
$css_classes[] = 'nt-theme-extra-row-bgposition-'.vc_shortcode_custom_css_class( $css ).'';
}

$nt_conversi_bg_ontablet  = ( $nt_conversi_bg_ontablet  != '' ) ? $nt_conversi_bg_ontablet  : 'show';
$nt_conversi_bg_onmobile  = ( $nt_conversi_bg_onmobile  != '' ) ? $nt_conversi_bg_onmobile  : 'show';
$nt_conversi_bg_onphone   = ( $nt_conversi_bg_onphone   != '' ) ? $nt_conversi_bg_onphone   : 'show';

if( $nt_conversi_bg_ontablet == 'hide' ) {
$css_classes[] = 'row-bg-image-hide-992';
}
if( $nt_conversi_bg_onmobile == 'hide' ) {
$css_classes[] = 'row-bg-image-hide-768';
}
if( $nt_conversi_bg_onphone == 'hide' ) {
...
if ( $nt_conversi_container_display == 'true' ) {
...
if( $nt_conversi_vc_row_992_responsive != '' || $nt_conversi_vc_row_768_responsive != '' || $nt_conversi_vc_row_480_responsive != '' || $nt_conversi_bg_position != '' ) {

	$nt_conversi_992row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$nt_conversi_vc_row_992_responsive);
	$nt_conversi_768row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$nt_conversi_vc_row_768_responsive);
	$nt_conversi_480row=preg_replace('/.vc_custom_[0-9]*/',esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ),$nt_conversi_vc_row_480_responsive);

	$nt_conversi_bgpositiony = ( $nt_conversi_bg_positiony != '' ) ? $nt_conversi_bg_positiony : '';
	$nt_conversi_bgpositionx = ( $nt_conversi_bg_positionx != '' ) ? $nt_conversi_bg_positionx : '';
	$nt_conversi_bgxoffset   = ( $nt_conversi_bg_xoffset   != '' ) ? $nt_conversi_bg_xoffset   : '';
	$nt_conversi_bgsize      = ( $nt_conversi_bg_size      != '' ) ? $nt_conversi_bg_size      : '';

	$nt_conversi_bgposition = ( $nt_conversi_bg_position == 'custom' ) ? ''.$nt_conversi_bg_positiony.' '.$nt_conversi_bg_positionx.'' : $nt_conversi_bg_position;

	$output .= '<style>';
		if( $nt_conversi_vc_row_992_responsive != '' ) {$output .= ' @media only screen and (max-width: 992px) { .'.esc_attr($nt_conversi_992row).' }'; }
		if( $nt_conversi_vc_row_768_responsive != '' ) {$output .= ' @media only screen and (max-width: 768px) { .'.esc_attr($nt_conversi_768row).' }'; }
		if( $nt_conversi_vc_row_480_responsive != '' ) {$output .= ' @media only screen and (max-width: 480px) { .'.esc_attr($nt_conversi_480row).' }'; }

		if( $nt_conversi_bgposition != '' ) {
			$output .= '.vc_row.nt-theme-extra-row-bgposition-'.vc_shortcode_custom_css_class( $css ).'{';
				if( $nt_conversi_bgposition != '' ) { $output .= 'background-position:'.$nt_conversi_bgposition.' '.$nt_conversi_bgxoffset.'!important;'; }
				if( $nt_conversi_bgsize     != '' ) { $output .= 'background-size: '.$nt_conversi_bg_size.' !important;'; }
			$output .= '}';
		}
	$output .= '</style>';
}
...
echo nt_conversi_vc_sanitize_data($output);
